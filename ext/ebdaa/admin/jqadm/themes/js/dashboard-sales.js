...
// Override and changed monthly graph data (changed order.base.product.price -> order.base.price)
...
Aimeos.Dashboard.Sales = {
    ...
    chartDay: function () {
        ...
        var xAxis = d3.axisBottom().scale(xScaleDays).ticks(width / days / 2).tickFormat(function (d) {
        ...
        $.when.apply($, Object.values(promises)).done(function () {
            ...
            svg.append("g")
                ...
                .attr("transform", function (d) {
                    return "translate(" + (xScaleDays(dateParser(d[0])) - width / days / 2) + ",0)";
                })
                .selectAll("rect")
                .data(function (d) {
    ...
    chartMonth: function () {
        ...
        months = d3.utcMonth.range(firstdate, new Date()).map(function (d) {
            return d.toISOString().substr(0, 7);
        });

        var xScaleMonths = d3.scaleBand().range([0, width]).domain(months).paddingInner(0.15);
        var xAxis = d3.axisBottom().scale(xScaleMonths).tickFormat(function (d) {
            return d.substr(5, 2);
        });
        ...
        let sum_of_monthly_orders = getSumOfMonthlyOrders()
        ...
        $.when.apply($, Object.values(promises)).done(function () {
            ...
            svg.append("g")
                .selectAll("g")
                .data(Object.entries(sum_of_monthly_orders))
                .enter().append("g")
                .attr("transform", function (d) {
                    return "translate(" + xScaleMonths(d[0]) + ",0)";
                })
                .selectAll("rect")
                .data(function (d) {
                    return Object.entries(d[1]);
    ...
    chartWeekday: function () {
        ...
        var xAxis = d3.axisBottom().scale(xScaleWdays).tickFormat(function (d) {
            return weekdays[d];
        });
        ...
        $.when.apply($, Object.values(promises)).done(function () {
            ...
            svg.append("g")
                ...
                .enter().append("g")
                .attr("transform", function (d) {
                    return "translate(" + xScaleWdays(d[0]) + ",0)";
                })
                .selectAll("rect")
                .data(function (d) {
                    return Object.entries(d[1]);
...
function getSumOfMonthlyOrders() {

    // get site name by regex from url
    let current_url = window.location.href
    let reg_pattern = '/admin/[a-zA-Z0-9-_]+\/';
    let locale_site = current_url.match(reg_pattern)[0]
    locale_site = locale_site.replace('/admin/', '')
    locale_site = locale_site.replace('/', '')

    let params = {'site': locale_site}
    ...
    $.ajax({
        ...
        success: function (data) {
            for (let month in data.data) {
                result[month] = {'QAR': data.data[month]}
            }
        }
    });
