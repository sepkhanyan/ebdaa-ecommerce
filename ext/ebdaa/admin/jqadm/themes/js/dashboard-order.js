...
Aimeos.Dashboard.Order = {
    ...
    chartDeliveryStatus : function() {

        var selector = "#order-countdeliverystatus-data",
            margin = {top: 20, bottom: 40, left: 50, right: 20},
            width = $(selector).width() - margin.left - margin.right,
            height = $(selector).height() - margin.top - margin.bottom - 10,
            bandwidth = 25;

        var statuslist = ['-1', '0', '1', '2', '3', '4', '5', '6','7'],
            statusrange = [
                "delivery-waiting", "delivery-cancelled", "delivery-ready-for-delivery", "delivery-preparing",
                "delivery-delivering", "delivery-delivered", "delivery-lost", "delivery-refused","delivery-returned"
            ];
        ...
        for(var i=0; i<statuslist.length; i++) {
            ...
            criteria = {"&&": [
                    {">": {"order.cdate": firstdate.toISOString().substr(0, 10)}},
                    {"==": {"order.statusdelivery": statuslist[i]}}
                ]};
        ...
        $.when.apply($, Object.values(promises)).done(function() {
            ...
            svg.append("rect")
                ...
                .on("mousemove", function() {
                    ...
                    tooltip.html(html)
                        .css("top", margin.top)
                        .css("left", (mouseX - width / 2 > 0 ? mouseX - tooltip.outerWidth() - 20 : mouseX + 50) + margin.left);
                });
