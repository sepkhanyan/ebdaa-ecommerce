                ...
                <div v-for="(item, idx) in items" v-bind:key="idx" class="group-item card">
                    ...
                    <div v-bind:id="'item-price-group-data-' + idx" v-bind:class="item['_show'] ? 'show' : 'collapsed'"
                        ...
                        <div class="col-xl-6">
                            ...
                            <div class="form-group row mandatory">
                                <label class="col-sm-4 form-control-label help"><?= $enc->html( $this->translate( 'admin', 'Cost price' ) ); ?></label>
                                <div class="col-sm-8">
                                    <input class="form-control item-value" type="number" step="any" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
                                           v-bind:name="'<?= $enc->attr( $this->formparam( array( 'price', 'idx', 'price.cost_price' ) ) ); ?>'.replace('idx', idx)"
                                           placeholder="<?= $enc->attr( $this->translate( 'admin', 'Cost price' ) ); ?>"
                                           v-bind:readonly="item['price.siteid'] != siteid"
                                           v-model="item['price.cost_price']" />
                                </div>
                                <div class="col-sm-12 form-text text-muted help-text">
                                    <?= $enc->html( $this->translate( 'admin', 'Cost price' ) ); ?>
                                </div>
                            </div>

                            <div class="form-group row mandatory">
                                <label class="col-sm-4 form-control-label help"><?= $enc->html( $this->translate( 'admin', 'Commission' ) ); ?></label>
                                <div class="col-sm-8">
                                    <input class="form-control item-value" type="number" step="any" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
                                           v-bind:name="'<?= $enc->attr( $this->formparam( array( 'price', 'idx', 'price.commission' ) ) ); ?>'.replace('idx', idx)"
                                           placeholder="<?= $enc->attr( $this->translate( 'admin', 'Commission' ) ); ?>"
                                           v-bind:readonly="item['price.siteid'] != siteid"
                                           v-model="item['price.commission']" />
                                </div>
                                <div class="col-sm-12 form-text text-muted help-text">
                                    <?= $enc->html( $this->translate( 'admin', 'Cost price' ) ); ?>
                                </div>
