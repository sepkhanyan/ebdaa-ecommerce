 ...
 * @category Developer
 */
$default = $this->config( 'admin/jqadm/product/fields', ['image', 'product.id', 'product.status', 'product.type', 'product.code', 'product.label', 'product.actual.price', 'product.basket_add_count', 'product.sold_count'] );
...
$columnList = [
    ...
    'product.actual.price' => $this->translate( 'admin', 'Actual Price' ),
    'product.basket_add_count' => $this->translate( 'admin', 'Basket Add Count' ),
    'product.sold_count' => $this->translate( 'admin', 'Sold Count' ),
...
<form class="list list-product" method="POST" action="<?= $enc->attr( $this->url( $target, $controller, $action, $searchParams, [], $config ) ); ?>">
	...
	<table class="list-items table table-hover table-striped">
		...
		<tbody>
			...
			<?= $this->partial(
				$this->config( 'admin/jqadm/partial/listsearch', 'common/partials/products-listsearch-standard' ), [
			...
			<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
				...
				<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ); ?>" data-label="<?= $enc->attr( $item->getLabel() ) ?>">
					...
					<?php endif; ?>
                    <?php if( in_array( 'product.actual.price', $fields ) ) : ?>
                        <td class="product-label">
                            <a class="items-field" href="<?= $url; ?>">
                                <?php foreach( $item->getListItems( 'price', null, null, false ) as $listItem ) : ?>
                                    <?php if($listItem->getRefItem()->getStatus() == 1):  ?>
                                        <?= $listItem->getRefItem()->getValue() ?> QAR
                                     <?php endif; ?>
                                     <?php break; ?>
                                <?php endforeach; ?>
                            </a>
                    ...
                    <?php endif; ?>
                    <?php if( in_array( 'product.basket_add_count', $fields ) ) : ?>
                        <td class="product-basket-count"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getBasketCount() ); ?></a></td>
                    <?php endif; ?>
                    <?php if( in_array( 'product.sold_count', $fields ) ) : ?>
                        <td class="product-sold-count"><a class="items-field" href="<?= $url; ?>"><?= $enc->html( $item->getSoldCount() ); ?></a></td>
                    <?php endif; ?>
