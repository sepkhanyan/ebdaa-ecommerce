...
$parentOrProductSite = false;
if( isset( $this->item )){
    $orderProducts = $this->item->getProducts();
    if(count($orderProducts) > 0){
        foreach ($orderProducts as $prod){
            if($prod->getSiteId() === $siteId){
                $parentOrProductSite = true;
            }else {
                $explodeProdSiteId = explode('.', $prod->getSiteId());
                $explodeSite = explode('.', $siteId);
                if(count($explodeProdSiteId) > count($explodeSite)){
                    $parentSite = $explodeSite[0] . $explodeSite[1];
                    $productParentSite = $explodeProdSiteId[0] . $explodeProdSiteId[1];
                    if($parentSite = $productParentSite){
                        $parentOrProductSite = true;
                    }
                }
            }
...
$order_customer_latitude = $this->get( 'itemData/address')['payment'][0]['order.base.address.latitude'];
$order_customer_longitude = $this->get( 'itemData/address')['payment'][0]['order.base.address.longitude'];
$order_customer_location = 'https://www.google.com/maps/search/?api=1&query=' . $order_customer_latitude . ',' . $order_customer_longitude;
?>
			...
			<div class="col-md-3 item-navbar">
				<ul class="nav nav-tabs flex-md-column flex-wrap d-flex justify-content-between" role="tablist">
					...
					<?php foreach( array_values( $this->get( 'itemSubparts', [] ) ) as $type => $subpart ) : ?>
						...
						<?php
							$paymentStatus = \DB::table('mshop_order')
							->select('statuspayment')
							->where('baseid', $basket->getId())
							->pluck('statuspayment')
							->first();
						?>
						<?php if($paymentStatus != 1 && $parentOrProductSite): ?>
                        <li class="nav-item <?= $enc->attr( $subpart ); ?>">
                            <a  class="nav-link" href="/xasy679834asd-invoice/<?= $enc->html( $basket->getId() ); ?>/<?=$siteId?>">Print invoice</a>
                        </li>
                        <?php endif ?>
				...
				<div id="order" class="item-order tab-pane fade show active" role="tabpanel" aria-labelledby="order">
					...
					<div class="row item-base">
						<div class="col-xl-6 content-block <?= $this->site()->readonly( $basket->getSiteId() ); ?>">
							...
							<!-- customer name added end -->

							<!-- email added -->
							<div class="form-group row">
								<label class="col-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'E-Mail' ) ); ?></label>
								<div class="col-8">
									<span><?= $enc->attr( $this->get( 'itemData/customer.email' ) ); ?></span>
								</div>
							</div>
                            <!-- phone added -->
                            <?php if($enc->attr( $this->get( 'itemData/customer.mobile' ) )): ?>
                                <div class="form-group row">
                                    <label class="col-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'Phone' ) ); ?></label>
                                    <div class="col-8">
                                        <span><?= $enc->attr( $this->get( 'itemData/customer.mobile' ) ); ?></span>
                                    </div>
                                </div>
                            <?php endif; ?>
                            <!-- phone end-->
                            ...
                            <!-- location added -->
                            <?php if($order_customer_latitude): ?>
                                <div class="form-group row">
                                    <label class="col-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'Location' ) ); ?></label>
                                    <div class="col-8">
                                        <a href="<?= $order_customer_location ?>" target="_blank">
                                            <?=substr($order_customer_location,0,60).'...';  ?>
                                        </a>
                                    </div>
                                </div>
                            <?php endif; ?>
						...
						<div class="col-xl-6 content-block <?= $this->site()->readonly( $basket->getLocale()->getSiteId() ); ?>">
							...
							<!-- customer id commented end -->

							<!-- customer address added -->
							<!-- <div class="form-group row">
								<label class="col-4 form-control-label"><?= $enc->html( $this->translate( 'admin', 'Customer Address' ) ); ?></label>
								<div class="col-8">
									<span class="form-control">
										<?= $enc->html( $this->translate( 'admin', 'Street' ) ); ?>: <?= $enc->attr( $this->get( 'itemData/customer.address1' ) ); ?>
									</span>
								</div>
								<div class="col-8">
									<span class="form-control">
									<?= $enc->html( $this->translate( 'admin', 'House number' ) ); ?>: <?= $enc->attr( $this->get( 'itemData/customer.address2' ) ); ?>
									</span>
								</div>
								<div class="col-8">
									<span class="form-control">
									<?= $enc->html( $this->translate( 'admin', 'Floor / Appartment' ) ); ?>: <?= $enc->attr( $this->get( 'itemData/customer.address3' ) ); ?>
									</span>
								</div>
								<div class="col-8">
									<span class="form-control">
									<?= $enc->html( $this->translate( 'admin', 'Zip code' ) ); ?>: <?= $enc->attr( $this->get( 'itemData/customer.postal' ) ); ?>
									</span>
								</div>
								<div class="col-8">
									<span class="form-control">
									<?= $enc->html( $this->translate( 'admin', 'City' ) ); ?>: <?= $enc->attr( $this->get( 'itemData/customer.city' ) ); ?>
									</span>
								</div>
								<div class="col-8">
									<span class="form-control">
									<?= $enc->html( $this->translate( 'admin', 'Country' ) ); ?>: <?= $enc->attr( $this->get( 'itemData/customer.countryid' ) ); ?>
									</span>
								</div>
							</div> -->
							<!-- customer address added end -->
					<!-- new address added -->
					<div class="form-group row">
						<?php foreach( $basket->getAddresses() as $type => $addresses ) : $code = 'address:' . $type; ?>

							<div class="col-xl-7 content-block item-address">
								...
								<?php foreach( $addresses as $pos => $addrItem ) : ?>
									...
									<fieldset class="address-form">
										...
										<div class="form-group row optional">
											<?php
												$addr1 = $this->get( 'itemData/address/' . $type . '/' . $pos . '/order.base.address.address1' );
												$addArr = explode(',', $addr1);
												$count = count($addArr);
												if ($count == 1) {
													$area = $this->get( 'itemData/address/' . $type . '/' . $pos . '/order.base.address.title' );
													$street = $addArr[0];
												}elseif ($count == 2) {
													$area = $addArr[0];
													$street = $addArr[1];
													...
													$area = $addArr[0];
													$street = $addArr[1].$addArr[2];
												}
											 ?>
											<label class="col-sm-4 form-control-label help"><?= $enc->html( $this->translate( 'admin', 'Area' ) ); ?></label>
											<div class="col-sm-8">
												<input class="form-control item-area" type="text" tabindex="1" data-field="area"
										...
										<div class="form-group row optional">
											...
											<div class="col-sm-8">
												<?php
													switch ($this->get( 'itemData/address/' . $type . '/' . $pos . '/order.base.address.company' )) {
														case '1-فيلا':
															$value = 'Villa';
															break;
														case '0-مكتب':
														...
														case '0-Office':
															...
															break;
														default:
															$value = $this->get( 'itemData/address/' . $type . '/' . $pos . '/order.base.address.company' );
															break;
													}
												?>
						...
						<!-- new address ends -->
							...
							<?php endif ?> -->
							<!-- Customer commented end -->

							<!-- Customer reference commented-->
							<!-- <div class="form-group row optional">
							...
							<!-- Customer reference commented ends -->
						</div>
					</div>
