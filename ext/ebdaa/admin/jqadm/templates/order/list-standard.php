...
$columnList = [
	...
	'order.cdate' => $this->translate( 'admin', 'Created' ),
	'order.ctime' => $this->translate( 'admin', 'End Date' ),
    ...
    'order.base.service.delivery.type' => $this->translate( 'admin', 'Delivery Type' ),
    'order.base.service.final.costs' => $this->translate( 'admin', 'Final Delivery Cost' ),
	...
	<table class="list-items table table-hover table-striped">
		<thead class="list-header">
			<tr>
				...
				<th class="actions">
					<a class="btn fa act-download all-orders-download-excel" tabindex="1"
						href=""
		...
		<tbody id="orders-list-tbody">
            <input type="hidden" name="site-code" value="<?= $this->param( 'site' ); ?>">
			<?= $this->partial(
				$this->config( 'admin/jqadm/partial/listsearch', 'common/partials/orders-listsearch-standard' ), [
					...
					'data' => [
                        ...
                        'order.base.service.delivery.type' => [],
                        'order.base.service.final.costs' => [],
			...
			?>
			<?php
				$currentSiteId = App\LocaleSite::getSiteIdFromCode($searchParams['site']);
				$parentId = App\LocaleSite::getParentIdFromCode($searchParams['site']);
			?>
			<?php foreach( $this->get( 'items', [] ) as $id => $item ) : ?>
				<?php
					$prodExist = App\OrderBaseProduct::validateProdsFromCode($item->getBaseId(), $parentId, $currentSiteId);
				?>
				<?php if($prodExist): ?>
				...
				<tr class="list-item <?= $this->site()->readonly( $item->getSiteId() ); ?>">
					...
					<?php if( in_array( 'order.base.id', $fields ) ) : ?>
						<td class="order-id"><a class="items-field" href="<?= $url; ?>" tabindex="1"><?= $enc->html( $item->getBaseId() ); ?></a></td>
					<?php endif; ?>
					...
					<?php endif; ?>
					<?php if( in_array( 'order.cdate', $fields ) ) : ?>
					...
					<?php endif; ?>
                    <?php if( in_array( 'order.base.service.delivery.type', $fields ) ) : ?>
                        <td class="order-base-service-delivery">
                            <a class="items-field" href="<?= $url; ?>">
                                <?php foreach( $services as $type => $list ) : ?>
                                    <?php foreach( $list as $serviceItem ) : ?>
                                        <span><?= $enc->html( $serviceItem->getDeliveryType() ); ?></span>
                                    <?php endforeach; ?>
                                <?php endforeach; ?>
                            </a>
                        </td>
                    <?php endif; ?>
                    <?php if( in_array( 'order.base.service.final.costs', $fields ) ) : ?>
                        <td class="order-base-service-final-costs">
                            <a class="items-field" href="<?= $url; ?>">
                                <?php if($services['delivery']) : ?>
                                    <?php foreach( $services['delivery'] as $type => $serviceItem ) : ?>
                                        <span><?= $enc->html( $serviceItem->getFinalCosts() ); ?></span>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </a>
                        </td>
