 ...
 * @category Developer
 */
$default = ['order.id', 'order.datepayment', 'order.statuspayment', 'order.statusdelivery', 'order.base.service.delivery.type'];
...
$deliveryTypes = [ 'Mr Delivery', 'Qpost', 'Vendor', 'Syaanh'];
...
<div id="invoice" class="item-invoice content-block tab-pane fade" role="tabpanel" aria-labelledby="invoice">
    ...
    <table class="list-items table table-striped table-hover">
        ...
        <tr>
            <?= $this->partial(
                ...
                'data' => [
                    ...
                    'order.base.service.name' => $this->translate( 'admin', 'Delivery By' ),
        ...
        <tbody>
        <?= $this->partial(
            $this->config( 'admin/jqadm/partial/listsearch', 'common/partials/orders-invoices-listsearch-standard' ), [
        ...
        <?php foreach( $this->get( 'invoiceData/order.id', [] ) as $idx => $orderId ) : ?>
            ...
            <tr class="list-item <?= $this->site()->readonly( $siteId ); ?>">
                ...
                <?php if( in_array( 'order.base.service.name', $fields ) ) : ?>
                     <td class="order-base-service-name"><?= $enc->html( $this->get( 'invoiceData/order.base.service.name/' . $idx ) ); ?></td>
                <?php endif; ?>
                <?php if( in_array( 'order.base.service.delivery.type', $fields ) ) : ?>
                    <td >
                        <select class="form-control custom-select delivery-type" required="required" tabindex="<?= $this->get( 'tabindex' ); ?>"
                                name="invoice[order.base.service.delivery.type][]"
                            <?= $this->site()->readonly( $siteId ); ?> disabled="disabled" >
                            <option value="">
                                <?= $enc->html( $this->translate( 'admin', 'Please select' ) ); ?>
                            </option>
                            <?php foreach( $deliveryTypes as  $deliveryType ) : ?>
                                <option value="<?= $enc->html( $deliveryType ); ?>" <?= $selected( $this->get( 'invoiceData/order.base.service.delivery.type/' . $idx ), $deliveryType ); ?> >
                                    <?= $enc->html( $deliveryType ); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </td>
                ...
                <?php if( in_array( 'order.base.service.final.costs', $fields ) ) : ?>
                     <td >
                         <input class="form-control item-taxrate"
                                type="number"
                                min="0"
                                step="0.01"
                                name="invoice[order.base.service.final.costs][]"
                                value="<?= $enc->html($this->get('invoiceData/order.base.service.final.costs/' . $idx))?>"
                                disabled="disabled"
                         />
                     </td>
                <?php endif; ?>
