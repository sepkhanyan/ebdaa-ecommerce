			...
			<td class="<?= str_replace( '.', '-', $key ); ?>">
				<?php if( $list !== null ) : ?>
					<input type="hidden" value="<?= $enc->attr( $key ); ?>"
						name="<?= $enc->attr( $this->formparam( array_merge( $group, ['filter', 'key', $idx] ) ) ); ?>" <?php if($key == 'product.actual.price') : ?> disabled <?php endif; ?>   />
					<input type="hidden" value="<?= $enc->attr( $this->value( $list, 'op', '=~' ) ); ?>"
						name="<?= $enc->attr( $this->formparam( array_merge( $group, ['filter', 'op', $idx] ) ) ); ?>" <?php if($key == 'product.actual.price') : ?> disabled <?php endif; ?>  />
					...
					<?php else : ?>
						<input class="form-control" type="<?= $enc->attr( $type ); ?>" tabindex="<?= $this->get( 'tabindex' ); ?>"
							name="<?= $enc->attr( $this->formparam( array_merge( $group, ['filter', 'val', $idx] ) ) ); ?>"
							value="<?= $enc->attr( $this->value( $filter, 'val/' . $idx, '' ) ); ?>" <?php if($key == 'product.actual.price') : ?> disabled <?php endif; ?>  />
					<?php endif; ?>
				<?php endif; ?>
			</td>
		<?php endif; ?>
