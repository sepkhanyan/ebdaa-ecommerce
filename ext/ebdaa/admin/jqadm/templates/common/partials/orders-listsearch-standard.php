        ...
        <?php if( in_array( $key, $fields ) ) : ?>
            <td class="<?= str_replace( '.', '-', $key ); ?>">
                <?php if( $list !== null ) : ?>
                    ...
                    <?php else : ?>
                        <?php if($key == 'order.ctime'): ?>
                            <input class="form-control" type="hidden" id="end_date_input"
                                   name="<?= $enc->attr( $this->formparam( array_merge( $group, ['filter', 'val', $idx] ) ) ); ?>"
                                   value="<?= $enc->attr( $this->value( $filter, 'val/' . $idx, '' ) ); ?>" />
                            <input class="form-control" type="<?= $enc->attr( $type ); ?>" name="orders-ctime"
                                   value="<?=   $enc->attr( $this->value( $filter, 'val/' . $idx, '' ) ) == '' ?
                                                $enc->attr( $this->value( $filter, 'val/' . $idx, '' ) ) :
                                                \Carbon\Carbon::create($enc->attr( $this->value( $filter, 'val/' . $idx, '' )))->subDay()->toDateString() ?>"
                                   />
                        <?php else: ?>
