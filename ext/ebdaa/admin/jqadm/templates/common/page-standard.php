...
//set dynamic logo
$subject = $site;
$pattern_syaanh = '/^syaanh/';
$pattern_mzad = '/^mzad/';
$pattern_fixingstrading = '/^fixingstrading/';

//regexp
preg_match($pattern_syaanh, substr($subject,0), $matches_syaanh);

preg_match($pattern_mzad, substr($subject,0), $matches_mzad);

preg_match($pattern_fixingstrading, substr($subject,0), $matches_fixingstrading);

$logo_path = '';
$syaanh_logo_class = '';
//check with regular expressions and set path
if((count($matches_syaanh) > 0 && $matches_syaanh[0] === 'syaanh')
    || (count($matches_fixingstrading) > 0 && $matches_fixingstrading[0] === 'fixingstrading')
    || $subject == 'default'
    || $subject == 'planettec'
    || $subject == 'zainalelectronics'
){
    $logo_path = asset('icons/512х512.png');
    $syaanh_logo_class = 'syaanh-logo';
}
elseif ((count($matches_mzad) > 0 && $matches_mzad[0] === 'mzad')){
    $logo_path = asset('icons/mzad.png');
}
else{
    $logo_path = "https://aimeos.org/check/?type=".$this->get( 'aimeosType' )."&version=".$this->get( 'aimeosVersion' )."&extensions=".$this->get( 'aimeosExtensions' );
}
?>
<div class="aimeos" lang="<?= $this->param( 'lang' ); ?>" data-url="<?= $enc->attr( $this->url( $jsonTarget, $jsonCntl, $jsonAction, array( 'site' => $site ), [], $jsonConfig ) ); ?>">
    ...
    <nav class="main-sidebar">
        <div class="sidebar-wrapper">
            ...
            <a class="logo <?= $syaanh_logo_class ?>" target="_blank" href="https://aimeos.org/update/?type=<?= $this->get( 'aimeosType' ) ?>&version=<?= $this->get( 'aimeosVersion' ) ?>">
                <img src="<?= $logo_path ?>" alt="Aimeos update" title="Aimeos update">
            </a>
