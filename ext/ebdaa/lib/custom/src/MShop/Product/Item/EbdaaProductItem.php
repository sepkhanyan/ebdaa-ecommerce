 ...
 * @subpackage Product
 */
class EbdaaProductItem extends Standard {
    ...
    /**
     * Returns the basket add count of the product item.
     *
     * @return int Basket add count of the product item
     */
    public function getBasketCount() : int
    {
        return $this->get( 'product.basket_add_count', 0);
    }


    /**
     * Sets the new basket add count of the product item.
     *
     * @param int $status New basket add count of the product item
     * @return \Aimeos\MShop\Product\Item\Iface Product item for chaining method calls
     */
    public function setBasketCount( int $status ) : \Aimeos\MShop\Common\Item\Iface
    {
        return $this->set( 'product.basket_add_count', 0 );
    }

    /**
     * Returns the sold count of the product item.
     *
     * @return int Sold add count of the product item
     */
    public function getSoldCount() : int
    {
        return $this->get( 'product.sold_count', 0);
    }


    /**
     * Sets the new sold count of the product item.
     *
     * @param int $status New sold count of the product item
     * @return \Aimeos\MShop\Product\Item\Iface Product item for chaining method calls
     */
    public function setSoldCount( int $status ) : \Aimeos\MShop\Common\Item\Iface
    {
        return $this->set( 'product.sold_count', 0 );
    }
    ...
    public function fromArray( array &$list, bool $private = false ) : \Aimeos\MShop\Common\Item\Iface
    {
        ...
        foreach( $list as $key => $value )
        {
            switch( $key )
            {
                ...
                case 'product.basket_add_count': $item = $item->setBasketCount( $value ); break;
                case 'product.sold_count': $item = $item->setSoldCount( $value ); break;
                default: continue 2;
            }
    ...
    public function toArray( bool $private = false ) : array
    {
        ...
        $list['product.basket_add_count'] = $this->getBasketCount();
        $list['product.sold_count'] = $this->getSoldCount();
